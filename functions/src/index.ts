...
export const sendMembershipWelcomeEmail = functions.https.onCall(async (data: { userId: string, userEmail: string, subscriptionId: string }, context) => {
  // Verify that the user is authenticated
  if (!context.auth) {
...
function getBookingCustomerEmail(booking: any): string {
  return booking.customerEmail || booking.customerInfo?.email || booking.email || booking.userEmail || '';
}

function getBookingCustomerName(booking: any): string {
...
async function sendDepositReminderEmail(booking: any, bookingId: string, config?: any) {
  try {
    ...
    await sendDepositReminderEmailViaServer({
      customerEmail,
      customerName: getBookingCustomerName(booking),
...
function getInflatableName(booking: any): string {
...
async function sendMembershipWelcomeEmailInternal(subscriptionData: any, subscriptionId: string, fallbackEmail?: string) {
  try {
...
async function sendMembershipConfirmationEmail(booking: any, bookingId: string) {
  try {
...
async function sendMembershipEventConfirmationEmail(booking: any, bookingId: string) {
  try {
...
async function sendMembershipPostEventThanksEmail(booking: any, bookingId: string) {
  try {
...
async function sendMembershipCancellationEmailInternal(membershipData: any, membershipId: string) {
  try {
...
function calculateNextSendTime(recurrence: string): number {
  const now = Date.now();
  let nextTime = now;
...
async function processDepositReminderEmails(db: admin.database.Database, now: number, config?: any): Promise<number> {
  try {
    console.log('📧 SCHEDULER: Checking deposit reminder emails...');
    
    const TIMING = config?.triggerDelay || (7 * 24 * 60 * 60 * 1000); // Default 7 days before event
    const bookingsRef = db.ref('bookings');
    const snapshot = await bookingsRef.once('value');
...
async function processEventConfirmationEmails(db: admin.database.Database, now: number, config?: any): Promise<number> {
  try {
    console.log('📧 SCHEDULER: Checking event confirmation emails...');
    
    const TIMING = config?.triggerDelay || (3 * 24 * 60 * 60 * 1000); // Default 3 days before event
    const bookingsRef = db.ref('bookings');
    const snapshot = await bookingsRef.once('value');
...
async function processPostEventEmails(db: admin.database.Database, now: number, config?: any): Promise<number> {
  try {
    console.log('📧 SCHEDULER: Checking post-event emails...');
    
    const TIMING = config?.triggerDelay || (24 * 60 * 60 * 1000); // Default 1 day after event
    const bookingsRef = db.ref('bookings');
    const snapshot = await bookingsRef.once('value');
...
async function processRebookingReminderEmails(db: admin.database.Database, now: number, config?: any): Promise<number> {
  try {
    console.log('📧 SCHEDULER: Checking rebooking reminder emails...');
    
    const TIMING = config?.triggerDelay || (9 * 30 * 24 * 60 * 60 * 1000); // Default 9 months after event
    const bookingsRef = db.ref('bookings');
    const snapshot = await bookingsRef.once('value');
...
async function processMembershipEventConfirmationEmails(db: admin.database.Database, now: number, config?: any): Promise<number> {
  try {
    console.log('📧 SCHEDULER: Checking membership event confirmation emails...');
    
    const TIMING = config?.triggerDelay || (3 * 24 * 60 * 60 * 1000); // Default 3 days before delivery
...
async function processMembershipPostEventEmails(db: admin.database.Database, now: number, config?: any): Promise<number> {
  try {
    console.log('📧 SCHEDULER: Checking membership post-event emails...');
    
    const TIMING = config?.triggerDelay || (24 * 60 * 60 * 1000); // Default 1 day after delivery
    const membershipBookingsRef = db.ref('bookings/membershipBookings');
    const snapshot = await membershipBookingsRef.once('value');
            ...
            if (eventDateStr) {
              try {
                ...
                if (eventDate <= today) {
                  ...
                  if (bookingData.customerInfo?.email && sendGridApiKey) {
                    try {
                      const msg = {
...
export const sendAccountDeletionEmail = functions.https.onCall(async (data: {
  ...
  try {
    ...
    const result = await sendAccountDeletionEmailViaServer({
...
export const deleteAccountData = functions.https.onCall(async (_data: unknown, context: any) => {
  if (!context.auth) {
  ...
  try {
    ...
    if (walletSnapshot.exists()) {
      walletBalance = Number(walletSnapshot.val()?.balance || 0);
    }
    ...
    if (bookingsSnapshot.exists()) {
      ...
      Object.keys(bookings).forEach((bookingId) => {
        ...
        if (booking?.customerID === userId || booking?.userId === userId || booking?.customerInfo?.userId === userId) {
    ...
    throw new functions.https.HttpsError('internal', error?.message || 'Failed to delete account data.');
  }
});

// Send membership cancellation confirmation email
export const sendMembershipCancellationEmail = functions.https.onCall(async (data: {
  membershipData: any;
  membershipId: string;
}, context) => {
  if (!context.auth) {
    ...
    try {
      ...
      if (subscriptionDoc.exists) {
        ...
        if (membershipData) {
          ...
          if (!membershipData.customerEmail && !membershipData.userEmail && context.auth?.token?.email) {
            membershipData.customerEmail = context.auth.token.email;
          }
          if (!membershipData.customerName && !membershipData.userName && context.auth?.token?.name) {
...
export const createMembershipBooking = functions.region('us-central1').https.onCall(async (data, context) => {
  ...
  try {
    ...
    const bookingData = {
      ...
      customerName: context.auth.token?.name || 'Jump Club Member',
      userName: context.auth.token?.name || 'Jump Club Member',
    ...
    throw new functions.https.HttpsError(
      'internal',
      'Failed to create membership booking',
      { error: error instanceof Error ? error.message : 'Unknown error' }
    );
  }
});
...
export const sendChatMessage = functions.https.onCall(async (data: {
  message: string;
  userEmail: string;
  userPhone?: string;
  userName: string;
 ...
 */
export const createPayPalBuyerAuthorizationOrder = functions.https.onCall(async (data, context) => {
  try {
    if (!context.auth) {
      throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    ...
    throw new functions.https.HttpsError(
      'internal',
      `Failed to create PayPal authorization order: ${error.message}`,
      { error: error.message }
    );
  }
});

export const capturePayPalAuthorizationPayment = functions.https.onCall(async (data, context) => {
  try {
    if (!context.auth) {
      throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    ...
    const capture = await captureAuthorizationPayment({
      ...
      amount,
      currency: data.currency || 'USD',
      finalCapture: data.finalCapture ?? false
    });
    ...
    return {
      ...
      amount: Number(capture.amount?.value || amount),
      currency: capture.amount?.currency_code || data.currency || 'USD',
    ...
    throw new functions.https.HttpsError(
      'internal',
      `Failed to capture PayPal authorization payment: ${error.message}`,
      { error: error.message }
    );
  }
});
 ...
 */
export const createPayPalAuthorizationHold = functions.https.onCall(async (data, context) => {
  try {
    if (!context.auth) {
      throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    ...
    if (!data.orderId || !data.vaultId) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'Order ID and vaulted card ID are required'
      );
    }
    ...
    throw new functions.https.HttpsError(
      'internal',
      `Failed to create PayPal authorization hold: ${error.message}`,
      { error: error.message }
    );
  }
});
 ...
 */
export const voidExpiredAuthorizationHolds = functions.pubsub.schedule('every 1 hours')
  .timeZone('America/New_York')
  .onRun(async () => {
    const rtdb = admin.database();
    const now = Date.now();
    ...
    try {
      ...
      for (const [bookingId, booking] of Object.entries(bookings)) {
        const bookingData = booking as any;
        const hold = bookingData?.paymentDetails?.authorizationHold;
