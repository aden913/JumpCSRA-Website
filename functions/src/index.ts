...
export const debugEnvironment = functions.https.onCall(async (data, context) => {
  return {
  ...
  try {
    ...
    switch (data.emailType) {
      ...
      case 'membership-confirmation':
        if (!data.bookingId) throw new Error('bookingId required for membership confirmation email');
      ...
      case 'membership-event-confirmation':
        if (!data.bookingId) throw new Error('bookingId required for membership event confirmation email');
      ...
      case 'membership-post-event-thanks':
        if (!data.bookingId) throw new Error('bookingId required for membership post-event email');
...
async function processMembershipEventConfirmationEmails(db: admin.database.Database, now: number) {
  try {
...
async function processMembershipPostEventEmails(db: admin.database.Database, now: number) {
  try {
...
export const testMembershipConfirmationEmail = functions.https.onCall(async (data, context) => {
  try {
    const testBooking = {
