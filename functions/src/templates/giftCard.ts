...
import { GiftCardEmailData } from '../types/email';

/**
 * Generate gift card email HTML
 */
export const generateGiftCardEmailHTML = (data: GiftCardEmailData): string => {
 ...
 */
export const generateGiftCardEmailText = (data: GiftCardEmailData): string => {
  const formatDate = (dateString: string) => {
    return new Date(dateString).toLocaleDateString('en-US', {
      year: 'numeric',
      month: 'long',
