...
ssh root@${SERVER} "cd ${SERVER_PATH}/serverFiles && npm install && pm2 restart jumpcsra-server || pm2 start server.js --name jumpcsra-server && pm2 save"

Write-Host "✅ Deployment completed!" -ForegroundColor Green
Write-Host "🌐 Your website should now be available at: https://jumpcsra.com" -ForegroundColor Cyan
Write-Host "📊 Check server status with: ssh root@${SERVER} pm2 status" -ForegroundColor Gray